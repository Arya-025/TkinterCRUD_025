...
def insert_nilai(nama, bio, fis, ing, prediksi):
    ...
    cur.execute('''
                ...
                biologi, fisika, inggris, prediksi_fskultas)
                VALUES (?,?,?,?,?)
    ''')
    conn.commit()
    conn.close()
...
def fetch_all():
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute (' SELECT id,nama_siswa,biologi, fisika, inggris, prediksi_fskultas FROM nilai_siswa ORDER BY id DESC')
    rows = cur.fetchall()
    conn.close
    return rows
...
class NilaiApp:
    def __init__(self, root):
        ...
        try:
            ...
            pass
        style.configure('TLabel', front=('segoe UI',10))
        style.configure('TButton', front=('segoe UI', 10), padding=6)
        style.configure('Header.TLabel', front=('segoe UI', 12, 'bold'))
        style.configure('Treeview.Heading', front=('segoe UI', 10, 'bold'))
        style.configure('Treeview', front=('segoe UI', 10))
