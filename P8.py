...
def update_nilai(id, nama, bio, fis, ing, prediksi):
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute('''
                UPDATE nilai_siswa SET nama_siswa = ?, biologi = ?, fisika = ?, inggris = ?, prediksi_fakultas = ?
                WHERE id = ?
...
class NilaiApp:
    def __init__(self, root):
        ...
        frm_left.grid(row=0, column=0, sticky='nsw', padx=12, pady=12)
        ...
        self.btn_update.grid(row=0, column=4, padx=(0, 8))
        ...
        # Bind event untuk memilih baris
        ...
        self.selected_id = None  # Untuk menyimpan ID yang dipilih
    ...
    def on_update(self):
        if self.selected_id is None:
            messagebox.showwarning('Peringatan', 'Pilih baris dari tabel untuk diupdate.')
            return
        ...
        messagebox.showinfo('Sukses', f'Data berhasil diupdate. Prediksi baru: {prediksi}')
        self.clear_form()
        self.selected_id = None
        self.load_table()
    ...
    def on_delete(self):
        if self.selected_id is None:
            messagebox.showwarning('Peringatan', 'Pilih baris dari tabel untuk dihapus.')
            return
